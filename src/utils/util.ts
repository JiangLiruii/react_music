...
function createWebAPIRequest(
  ...
  const is_csrf_cookie = cookie.match(/_csrf=[^(;|$)]+/g)
  if (is_csrf_cookie) {
    data.csrf_token = is_csrf_cookie[0].slice(6);
  } else {
    data.csrf_token = ''
  }
  // cookie 与 proxy 抽离
  ...
  const crypto_req = Encrypto(data);
  const options = {
    url: `http://${host + path}`,
    method,
    headers: {
    ...
    body: querystring.stringify({
      params:crypto_req.params,
      encSecKey:crypto_req.encSecKey
    }),
    proxy,
  }
  console.log(
